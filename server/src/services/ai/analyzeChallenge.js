...
const analyzeChallengeText = async (challenge) => {
  const response = await gemini.models.generateContent({
    ...
    contents: buildUserPrompt(challenge),
    config: {
  ...
  if (!parsed.category || !Array.isArray(parsed.keywords)) {
    throw new Error('Model response was missing required fields');
  }

  // Priority is calculated deterministically, not taken from the model
  const { score, priority } = calculatePriority(
    challenge,
    parsed.keywords
  );
